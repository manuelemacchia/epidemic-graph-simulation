...
class Graph:
    def __init__(self, graph_type, *args):
        self.graph = getattr(self, graph_type)(*args)

    def k_regular(self, k, n):
        """Generate a symmetric k-regular graph with n nodes.

        :param k: neighbors of each node (even, k/2 on the left and k/2 on the right)
    ...
    def preferential_attachment(self, k, n):
        ...
        :param k: average degree of the final graph
        :param n: total number of nodes of the final graph
        """
        ...
        for n_t in range(len(graph.nodes), n):
            ...
            # Note that, in case k is odd, then k/2 will not be an integer,
            # thus we alternate ceil(k/2) and floor(k/2) to achieve over
            # a large number of nodes an average degree of k/2.
            # This logic has no effect when k is even.
            w_n_t = int(np.ceil(k/2) if n_t % 2 == 0 else np.floor(k/2))

            # Create w_n_t new (undirected) edges to node n_t, according to
            # the probabilities we previously computed. We do not add multiple
            # links to the same node (take w_n_t samples from [0, n_t) without
